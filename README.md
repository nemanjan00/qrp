...
[![core size](https://img.shields.io/badge/core-~4.8_KB_min%2Bgzip-ffb23e?style=flat-square&labelColor=0a0d12)](#-performance)
...
`Proxy`, the DOM is real, and the published core is **~4.8 KB min+gzip** — the
number that lines up next to Solid (~7 KB) or React (~45 KB). The whole library,
every module, is **~22.5 KB min+gzip**.
...
Because the whole library is ~22.5 KB min+gzip, a useful qrp app doesn't need a
...
**[full example →](https://github.com/nemanjan00/qrp/tree/master/examples/esp32)** (PlatformIO project + build)
